...
parser.add_argument('--nomediaccc', action='store_true', default=False)
parser.add_argument('--nobaseurl', action='store_true', default=False)
...
mediaccc = not args.nomediaccc
baseurl = not args.nobaseurl
...
# frab_base_url = 'https://frab.fossgis-konferenz.de/'
...
def main():
  for event_id in schedule.xpath(u'day/room/event/@id'):
    ...
    try:
      ...
      urllib.urlretrieve(file_url, "temp/{0}/{0}.mp4".format(slug))
    except:
    ...
    for p in event.find('persons'):
      p = p.text.split(' ')
      if len(p) > 3:
        print("   \033[91mWARNING: Person name consists of more then three parts: " + str(p) + "\033[0m")
      if len(p) == 1:
        persons.append(p[0])
      elif len(p) == 2:
    ...
    if dry_run:
      if not os.path.exists("temp/{0}".format(slug)):
        os.mkdir("temp/{0}".format(slug))
    with open("temp/{0}/{0}.xml".format(slug), "wt") as f:
...
def find_recoding(guid):
  ...
  if guid_exist == False:
    ...
    for r in voctoweb_event['recordings']:
      ...
      if r['folder'] == 'h264-hd' and r['mime_type'] == 'video/mp4' and r['language'] == voctoweb_event[
        'original_language']:
