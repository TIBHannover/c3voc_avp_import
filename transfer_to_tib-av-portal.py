...
def main():
    for event_id in schedule.xpath(u'day/room/event/@id'):
        ...
        if os.path.isfile('temp/{0}/_{0}_.pdf'.format(slug)):
            ...
            if not dry_run:
                with host.open("{0}/_{0}_.pdf".format(slug), "wb") as f:
                    shutil.copyfileobj(u, f)
        ...
        with f:
            ...
            <title titleType="Subtitle" language="''' + lang + '''">''' + (
                        cgi.escape(str(event.find('subtitle').text)) or '') + '''</title></titles>
            <creators>''' + '\n    '.join(
                ['<creator><creatorName>{}</creatorName></creator>'.format(cgi.escape(str(p))) for p in persons]) + '''</creators>
